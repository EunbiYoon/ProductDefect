...
######################################## [i,j]
Title=[['1. F/L Cabinet cover Gap','2. F/L Noise test – no issue','3. F/L UE Error retest – no issue'],
       ['4. F/L Control panel Gap','5. F/L Rotor noise test – no issue','6. F/L Bellows leakage retest- no issue'],
       ['7. T/L Motor noise retest – no issue','8. T/L Top cover Gap','9. T/L PCB Touch button'],
          ['10. T/L Bad Spin Inner tub']]
...
for i in range(4):
    for j in range(3):
        ...
        if tri_value<10:
                print(str(i)+str(j))
                data=pd.read_excel('//us-so11-na08765/R&D Secrets/Q-bank/Daily Report/PGM File2.xlsx',sheet_name=str(tri_value))
                data=data.T
                print(data)
